...
def scrubMany(args):
    ...
    if args.model_file is not None:
        outString, _ = os.path.splitext(args.model_file)
    else:
        outString = 'trained_models/'+args.dataset+"_"+args.model+'_ntp_'+str(args.used_training_size)+'_seed_' + str(args.train_seed)+'_epochs_' + str(args.train_epochs)+'_lr_' + str(args.train_lr)+'_wd_' + str(args.train_wd)+'_bs_' + str(args.train_bs)+'_optim_' + str(args.train_optim)
    ...
    ordering_path = outString + "_ordering.npy"
    if os.path.isfile(ordering_path):
        ordering = np.load(ordering_path)
        if args.used_training_size is not None:
            ordering = ordering[:args.used_training_size]
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--MODEL_FILE', dest='model_file', type=str, default=None, help='Explicit path to a trained model file')
    ...
    arg_parser.add_argument('--train_seed', type=int, default=0, help='Seed used during original training (for filename matching)')
    arg_parser.add_argument('--used_training_size', type=int, default=1000, help='Training set size used during original training (for filename matching)')
    ...
    arg_parser.add_argument('--orig_trainset_size', type=int, default=50000, help='size of orig training set')
